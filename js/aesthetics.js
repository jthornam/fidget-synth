...
vec3 art(vec2 uv, float A, float B, float mode, float t, float shift) {
  ...
  if (mode < 0.5) {
    ...
    // Silk flow: fbm stretched hard along one axis. A: scale. B: grain angle.
    float aa = B * 3.1416;
    vec2 dir = vec2(cos(aa), sin(aa));
    vec2 per = vec2(-dir.y, dir.x);
    vec2 q = vec2(dot(uv, dir) * sc * 0.5, dot(uv, per) * sc * 2.5);
...
vec3 art(vec2 uv, float A, float B, float mode, float t, float shift) {
  ...
  if (mode < 0.5) {
    ...
    // Circle punch. A: disc scale. B: composition rotation.
    float ca = B * 3.1416;
    vec2 p = mat2(cos(ca), -sin(ca), sin(ca), cos(ca)) * uv;
    float acc = 0.0;
    ...
    for (int i = 0; i < 6; i++) {
      ...
      acc += 1.0 - smoothstep(rr - aaw, rr + aaw, length(p - c));
    }
    col = gcol(acc, shift);
